...
public class PersistEntity {
	...
	public PersistEntity(Entity entity)
	{
		m_entity = entity;
		...
		{ 	
    		...
    		{                	
    			...
    			if(!isComplexTable)
    				...
    				paraFilter = paraFilter.replace("@paraColumn@", m_entity.getName() + "." + attName);
    		}
    		else
    		{
        		if(val.getMultiplicity() != null && !val.getMultiplicity().equals("")){
    			...
    			if(!isComplexTable)
    				...
    				paraFilter = paraFilter.replace("@paraColumn@", m_entity.getName() + "." + attName + "ID");
	...
	public String genIbaitsMapper()
	{
		...
		{ 	
    		...
    		else
    		{
        		...
        		//	complexTableNames +=   "," + getTableName(typeEntity) + " as " + attName + "_" + typeEntity.getName();
        			...
        			String joinedTable =  getTableName(typeEntity) + " as " + attName + "_" + typeEntity.getName();
        			
        		//	if(!joinCode.equals(""))
        		//		joinCode += " and ";
        			joinCode += " left join " + joinedTable + " on " +m_entity.getName() + "." + columnName + "ID = " + attName + "_" + typeEntity.getName() + ".ID" ; 
		...
		if(hasComplexData == false)		
			...
			+ joinCode + " where " +  m_entity.getName()  + ".id = #{id} \")" ;
		...
		if(hasComplexData == false)		
			...
			daoCode += "\r\n\r\n\t@Select(\"select " + selectTarget + " from " + complexTableNames +  joinCode + " ${condition}\")" ;
